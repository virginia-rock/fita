...
export async function createInsiderCheckoutSession(): Promise<{ url: string }> {
  if (!supabase) throw new Error("O Supabase não está configurado para iniciar o pagamento.");
  const { data, error } = await supabase.functions.invoke<{ url?: unknown; error?: unknown }>(
  ...
  if (
    ...
    throw new Error(
      typeof data?.error === "string" ? data.error : "Não foi possível iniciar o Checkout Insider.",
    );
  }
  return { url: data.url };
}
...
export async function cancelStripeSubscription(): Promise<void> {
  if (!supabase) throw new Error("O Supabase não está configurado para cancelar a assinatura.");
  const { data, error } = await supabase.functions.invoke<{ canceled?: unknown; error?: unknown }>(
    "cancel-stripe-subscription",
  );
  if (error || data?.canceled !== true) {
    throw new Error(
      typeof data?.error === "string" ? data.error : "Não foi possível cancelar a assinatura.",
    );
  }
}
